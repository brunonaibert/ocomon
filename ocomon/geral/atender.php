...
*/session_start();
	...
	if (isset($_POST['submit']))
	{
		...
		$resultado = mysql_query($queryA) or ($data_error[] = mysql_error() . "<br/>" . $queryA);
		...
		$resultado2 = mysql_query($query2) or ($data_error[] = mysql_error() . "<br/>" . $query2);
		...
		else
		{
			...
			if ($regDoc1 >0) {
				...
				$execDoc =mysql_query($sqlDoc) or ($data_error[] = mysql_error() . "<br/>" . $sqlDoc);
			} else {
			...
			if ($status != $row['status_cod']) { //O status foi alterado
				...
				$exec_sql = mysql_query($sql_ts_anterior) or ($data_error[] = mysql_error() . "<br/>" . $sql_ts_anterior);

				if ($exec_sql == false) $error= " erro 1";
				...
				if ($achou >0){ //esse status já esteve setado em outro momento
					...
					$exec_upd = mysql_query($sql_upd) or ($data_error[] = mysql_error() . "<br/>" . $sql_upd);
					...
					$exec_ins = mysql_query($sql_ins) or ($data_error[] = mysql_error() . "<br/>" . $sql_ins);

					if ($exec_ins == false) $error.= " erro 3 ";
				}
				...
				$exec_sql = mysql_query($sql_ts_novo) or ($data_error[] = mysql_error() . "<br/>" . $sql_ts_novo);

				if ($exec_sql == false) $error.= " erro 4";
				...
				if ($achou_novo > 0) { //status já existe na tabela tempo_status
					...
					if ($exec_upd == false) $error.= " erro 5";
					...
					if ($exec_ins == false) $error.= " erro 6 ";
				}
			}
