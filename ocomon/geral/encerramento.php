...
print "<div id='loading' class='loading' STYLE='display:none'></div>";
...
print "</select>";*/
    ...
    print "<div id='idLoad' class='loading'></div>";
...
if (isset($_POST['submit'])) {
    ...
    $rowSis = mysql_fetch_array($sis_idB) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $queryB);

    $queryC = "SELECT local from localizacao where loc_id = " . $_POST['loc'] . "";
    $loc_idC = mysql_query($queryC) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $queryC);
    $setor = mysql_result($loc_idC, 0);

    $queryD = "SELECT nome from usuarios where login like '" . $_SESSION['s_usuario'] . "'";
    $loginD = mysql_query($queryD) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $queryD);
    ...
    if (isset($_POST['alimenta_banco']) && $_POST['alimenta_banco'] == "SIM") {
        ...
        $resultado = mysql_query($query) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $query);
        ...
        $resultado1 = mysql_query($query1) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $query1);
        //------------------------------------------------------------- INICIO ALTERACAO --------------------------------------------------------------
    } else {
    ...
    if (isset($_POST['justificativa']) && $row_config['conf_desc_sla_out']) {
        ...
        $execJust = mysql_query($queryJust) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $queryJust);
    }
    //REMOVE O NÚMERO DO CHAMADO DA TABELA DE CHECAGEM DO SLAS
    $qryClear = "DELETE FROM sla_out WHERE out_numero = " . $_POST['numero'] . "";
    $execClear = mysql_query($qryClear) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $qryClear);
    //----------------------------------------------

    $status = 4; //encerrado
    if ($atendimento == null) {
        $query2 = "UPDATE ocorrencias SET status=" . $status . ", local=" . $_POST['loc'] . ", problema ='" . $radio_prob . "', " .
        "operador=" . $_SESSION['s_uid'] . ", instituicao='" . $_POST['inst'] . "', equipamento=" . (!empty($_POST['etiqueta']) ? (int)$_POST['etiqueta'] : 'null') . ", " .
    ...
    $resultado2 = mysql_query($query2) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $query2);

    // if (($resultado == false) or ($resultado1 == false) or ($resultado2 == false)) {
    if ($resultado == false) {
        ...
        $execDoc1 = mysql_query($sqlDoc1) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $sqlDoc1);
        ...
        if ($regDoc1 > 0) {
            ...
            $execDoc = mysql_query($sqlDoc) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $sqlDoc);
        } else {
        ...
        if ($status != $rowABS['status_cod']) { //O status foi alterado
            ...
            $exec_sql = mysql_query($sql_ts_anterior) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $sql_ts_anterior);
            ...
            if ($achou > 0) { //esse status já esteve setado em outro momento
                ...
                $exec_upd = mysql_query($sql_upd) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $sql_upd);
                if ($exec_upd == 0) {
                    $error .= " erro 2";
                }
                ...
                $exec_ins = mysql_query($sql_ins) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $sql_ins);
                if ($exec_ins == false) {
        ...
        $execfull = mysql_query($qryfull) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $qryfull);
        ...
        $execconf = mysql_query($qryconf) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $qryconf);
        ...
        if (isset($_POST['mailAR'])) {
            ...
            $execmsg = mysql_query($qrymsg) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $qrymsg);
        ...
        if (isset($_POST['mailUS'])) {
            ...
            $execMailUs = mysql_query($sqlMailUs) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $sqlMailUs);
            ...
            $execresposta = mysql_query($qryresposta) or ($data_error[] = __LINE__ ."<br/>". mysql_error() . "<br/>" . $qryresposta);
...
<script type="text/javascript">
	...
	function valida(){
		...
		if (ok) {
			_('loading').style.display = 'inline';
		}
