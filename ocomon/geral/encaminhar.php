...
if (!isset($_POST['submit'])) {
    ...
    if ($_SESSION['s_nivel'] == 1) {
        $linkEdita = "<br><b><a href='altera_dados_ocorrencia.php?numero=" . $_GET['numero'] . "'>" . TRANS('FIELD_EDIT_FOR_ADMIN') . ":</a></b><br>";
    } else {
    ...
    if ($row['status'] == 4) {$stat_flag = "";} else {
...
print "</TD>";
    ...
    if ($linhas2 > 0) { //ASSENTAMENTOS DO CHAMADO
        ...
        while ($rowAssentamento = mysql_fetch_array($resultado2)) {
            ...
            if ($rowAssentamento['asset_privated'] == 1) {
                $asset_checked = " checked";
            } else {
                $asset_checked = "";
            }
...
if (isset($_POST['submit']) && $_POST['submit'] == TRANS('BT_OK')) {
    ...
    if (!$erro) {
        ...
        { //$status!=1 and
            ...
            {
				//". (isset($_POST['etiq']) && !empty($_POST['etiq']) ? (int) $_POST['etiq'] : 'null') ."
				

                $query = "UPDATE ocorrencias SET operador=" . dbField($_POST['operador']) . ", problema = " . $catProb . ", instituicao=" . dbField($_POST['institui']) . ", equipamento = " . dbField($_POST['etiq']) . ", sistema = '" . $_POST['sistema'] . "', local=" . $_POST['local'] . ", data_fechamento=NULL, status=" . $depois . ", data_atendimento='" . date('Y-m-d H:i:s') . "', " .
                "data_abertura = '" . $data_agendamento . "', oco_real_open_date='" . $realOpenDate . "', oco_scheduled=" . $agendado . ", descricao='" . $description . "', contato='" . noHtml($_POST['contato']) . "', telefone='" . noHtml($_POST['ramal']) . "', oco_prior='" . $_POST['prioridade'] . "' WHERE numero=" . $_POST['numero'] . "";
                $resultado4 = mysql_query($query) or die("Linha: " . __LINE__ . "<br/>" . mysql_error() . "<br/>" . $query);
            } else {
                $query = "UPDATE ocorrencias SET operador=" . dbField($_POST['operador']) . ", problema = " . $catProb . " , instituicao=" . dbField($_POST['institui']) . ", equipamento = " . dbField($_POST['etiq']) . ", sistema = '" . $_POST['sistema'] . "', local=" . $_POST['local'] . ", data_fechamento=NULL, status=" . $depois . ", " .
                "data_abertura = '" . $data_agendamento . "', oco_real_open_date='" . $realOpenDate . "', oco_scheduled=" . $agendado . ", descricao='" . $description . "', contato='" . noHtml($_POST['contato']) . "', telefone='" . noHtml($_POST['ramal']) . "', oco_prior='" . $_POST['prioridade'] . "' WHERE numero=" . $_POST['numero'] . "";
                $resultado4 = mysql_query($query) or die("Linha: " . __LINE__ . "<br/>" . mysql_error() . "<br/>" . $query);
            }
        } else {
            if (($_POST['data_atend'] == "") and ($depois != 4) and (isset($_POST['resposta']))) //para verificar se já foi setada a data do inicio do atendimento. //Se eu incluir um assentamento seto a data de atendimento
            {
                ...
                $query = "UPDATE ocorrencias SET operador=" . dbField($_POST['operador']) . ", problema = " . $catProb . ", instituicao=" . dbField($_POST['institui']) . ", equipamento = " . dbField($_POST['etiq']) . ", sistema = '" . $_POST['sistema'] . "', local=" . $_POST['local'] . ", status=" . $depois . ", " .
                "data_abertura = '" . $data_agendamento . "', oco_real_open_date='" . $realOpenDate . "', oco_scheduled=" . $agendado . ", descricao='" . $description . "', contato='" . noHtml($_POST['contato']) . "', telefone='" . noHtml($_POST['ramal']) . "', oco_prior='" . $_POST['prioridade'] . "' WHERE numero=" . $_POST['numero'] . "";
                $resultado4 = mysql_query($query) or die("Linha: " . __LINE__ . "<br/>" . mysql_error() . "<br/>" . $query);
            }
        }

        if (($resultado3 == 0) or ($resultado4 == false)) {
...
<script type="text/javascript">
	...
	function valida(){
		...
		if (ok) var ok = validaForm('idRamal','ALFAFULLESPACO','<?php print TRANS('OCO_FIELD_PHONE')?>',1);
