<?php session_start();
  ...
  */
	...
	<link href="../../includes/javascript/select2/dist/css/select2.min.css" rel="stylesheet" />
	<script src="../../includes/javascript/select2/dist/js/select2.min.js"></script>
	<script src="../../includes/javascript/select2/dist/js/i18n/pt-BR.js"></script> -->
	...
	if ((!empty($rowconf) && !$rowconf['conf_user_opencall'])) {
			...
			$msgDisable = TRANS('MSG_DISABLED_OPENCALL');
			//print "<script>mensagem('" . $msgDisable . "'); redirect('abertura.php');</script>";
			echo "<br/><br/>";
...
##################################################
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_tag']) || empty($rowconf)) {
			...
			if ((!empty($rowconf) && $rowconf['conf_scr_chkhist']) || empty($rowconf)) {
				...
				print "<a class='likebutton' onClick=\"checa_chamados()\" title='".TRANS('CONS_CALL_EQUIP')."'><font color='#5E515B'>".TRANS('OCO_FIELD_HIST')."</font></a>";
			}
			print "</TD>";
		} else {
			$equipamento = null;
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_local']) || empty($rowconf)) {
			print "<TD width='20%' align='left' bgcolor=".TD_COLOR.">".TRANS('OCO_FIELD_LOCAL','Local').": ";
				...
				if ((!empty($rowconf) && $rowconf['conf_scr_btloadlocal']) || empty($rowconf)) {
 					...
 					print "<input type='button' class='btn btn-blue btn-square' id='idBtCarrega' title='".TRANS('LOAD_EQUIP_LOCAL')."' value='Carregar Local do equipamento' ".
 							"onClick=\"ajaxFunction('idDivSelLocal', 'showSelLocais.php', 'idLoad', 'unidade=idUnidade', 'etiqueta=idEtiqueta');\"".
 							"style=\"align:center; valign:middle; value='' name='carrega'\">"; // width:19px; height:19px; background-image: url('../../includes/icons/kmenu-hack.png'); background-repeat:no-repeat;\"
...
//                 	print "</TD>";
		...
		if (isset($_POST['OK'])==TRANS('BT_OK')) {
			...
			while ($i<=$replicate) //'".noHtml($descricao)."'
			{
					...
					if (!$schedule){
						$query.="".$_POST['instituicao'].",". (isset($_POST['equipamento']) && !empty($_POST['equipamento']) ? (int) $_POST['equipamento'] : 'null') .",'".$sistema."',".
						"'".noHtml($_POST['contato'])."','".noHtml($_POST['telefone'])."',".$_POST['local'].",".$operator.",".
...
<script type="text/javascript">
	...
	// 	//$('.select3').addClass('js-example-basic-single');
	...
	// 	$("select[name='sistema']").addClass('js-example-basic-single');
		...
		if (!LOAD) {
			...
			if (ok){
				...
				if (fone != null){
					...
					var ok = validaForm('idTelefone','ALFAFULLESPACO','<?php print TRANS('OCO_FIELD_PHONE')?>',1);
				} else ok = true;
				//}
			}
			if (ok){
