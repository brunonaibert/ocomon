  ...
  */session_start();
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") )&& !isset($_POST['submit'])) {
		...
		print "</td>";
                ...
                print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text'  class='text' name='saida' id='idDataSaida' value='".date("d/m/Y")."' autocomplete='off'></TD>";

		print "</tr>";

        	print "<TR>";
                print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('COL_DATE_DEV').":</TD>";
                print "<TD width='30%' colspan='3' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text'  class='text' name='volta' id='idDataDevolucao' value='".date("d/m/Y")."' autocomplete='off'></TD>";
        	print "</TR>";
	...
	if ($_POST['submit'] == TRANS('BT_CAD')){
		...
		if (!$erro)
		{
			...
			$query = "INSERT INTO emprestimos (material, responsavel, data_empr, data_devol, quem, local, ramal) values".
				...
				"'".$_POST['quem']."', '".$_POST['local']."', '".noHtml($_POST['ramal'])."')";
			$resultado = mysql_query($query) or die (TRANS('ERR_QUERY').'<br>'.$query);
	...
	if ($_POST['submit'] == TRANS('BT_ALTER')){
                ...
                $query2 = "UPDATE emprestimos SET material='".noHtml($_POST['material'])."', responsavel='".noHtml($_POST['responsavel'])."', ".
                	"ramal = '".noHtml($_POST['ramal'])."', local = ".$_POST['local'].", data_empr='".datam($_POST['saida'])."', data_devol='".datam($_POST['volta'])."', ".
