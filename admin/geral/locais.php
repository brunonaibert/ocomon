...
// 		print "<TABLE border='0' align='left' width='100%' bgcolor=".BODY_COLOR.">";
	...
	if ((isset($_GET['action'])  && $_GET['action']=="alter") && (!isset($_POST['submit']))) {
		...
		print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'>";
			...
			$sqlReitorias = "SELECT * FROM reitorias ORDER BY reit_nome ";
			$sqlReitResult = mysql_query($sqlReitorias);

			while ($rowReit = mysql_fetch_array($sqlReitResult)) {
				print "<option value = '" . $rowReit['reit_cod'] . "' ";
				print ($rowReit['reit_cod'] == $row['loc_reitoria'] ? "selected" : '');
				print ">" . $rowReit['reit_nome'] . "</option>";
			}


		
			// if (!empty($row['loc_reitoria'])) {
		...
		print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'>";
        	print "<select class='select'  name='predio'>";
				...
				$sqlPredios = "SELECT * FROM predios ORDER BY pred_desc ";
				$sqlPrediosResult = mysql_query($sqlPredios);

				print "<option value='-1'>".TRANS('SEL_BUILDING')."</option>";
				while ($rowPredio = mysql_fetch_array($sqlPrediosResult)) {
					print "<option value = '" . $rowPredio['pred_cod'] . "' ";
					print ($rowPredio['pred_cod'] == $row['loc_predio'] ? "selected" : '');
					print ">" . $rowPredio['pred_desc'] . "</option>";
				}
		...
		print "<select  class='select' name='dominio'>";
			...
			$sqlDom = "SELECT * FROM dominios ORDER BY dom_desc ";
			$sqlDomResult = mysql_query($sqlDom);

			while ($rowDom = mysql_fetch_array($sqlDomResult)) {
				print "<option value = '" . $rowDom['dom_cod'] . "' ";
				print ($rowDom['dom_cod'] == $row['loc_dominio'] ? "selected" : '');
				print ">" . $rowDom['dom_desc'] . "</option>";
			}			
