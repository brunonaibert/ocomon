<?php 
  ...
  */session_start();
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<tr class='header'><td class='line'>".TRANS('TTL_EVENT','EVENTO').":</td><td class='line'>".$row['msg_event']."</td></tr>";
		print "<tr><td class='line'>".TRANS('OPT_FROM','FROM')."</td><td class='line'><input type='text' class='text' name='from' value='".$row['msg_fromname']."'></td></tr>";
		print "<tr><td class='line'>".TRANS('OPT_REPLY_TO','Responder para')."</td><td class='line'><input type='text' class='text' name='replyto' value='".$row['msg_replyto']."'></td></tr>";


		print "<tr><td class='line'>".TRANS('OPT_SUBJECT','Assunto')."</td><td class='line'><input type='text' class='text' name='subject' value='".$row['msg_subject']."'></td></tr>";
		//print "<tr><td>Msg HTML</td><td><textarea name='body' class='textarea2'>".$row['msg_body']."</textarea></td></tr>";
		print "<tr><td class='line'>".TRANS('OPT_HTML_MSG','Msg HTML')."</td><td>";
		

		print "<textarea name='body' class='textarea2'>".$row['msg_body']."</textarea>";
		...
		<?php 
		print "</td></tr>";
		print "<tr><td class='line'>".TRANS('OPT_ALTERNATE_MSG','Msg Alternativa')."</td><td><textarea name='altbody' class='textarea2'>".$row['msg_altbody']."</textarea></td></tr>";


		print "<tr><td class='line'><input type='submit'  class='button' name='submit' value='".TRANS('BT_ALTER','Alterar')."'></td>";
		print "<input type='hidden' value='".$_GET['event']."' name='event'>";
		print "<td class='line'><input type='reset' name='reset' class='button'  value='".TRANS('BT_CANCEL','Cancelar')."' onclick=\"redirect('configmsgs.php')\"></td></tr>";
