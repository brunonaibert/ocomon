<?php 
  ...
  */session_start();
		...
		"<td colspan='4'>".
			...
			print "<SELECT class='select' name='id_sistema' size='1'>";
				...
				while ($rowArea=mysql_fetch_array($execarea)){
					$isSelecionado = "";
					if ($rowArea['sis_id'] == (isset($_SESSION['id_sistema_filtro']) && !empty($_SESSION['id_sistema_filtro']) ? $_SESSION['id_sistema_filtro'] : ''))
		...
		if ($registros > 1) {
			...
			while ($row=mysql_fetch_array($resultado)) {
				...
				if ($_SESSION['s_nivel']!=3 || $row['scpt_enduser']==1){			
					print "<tr class='".$trClass." pointer' id='linhax".$j."' onMouseOver=\"destaca('linhax".$j."','".$_SESSION['s_colorDestaca']."');\" onMouseOut=\"libera('linhax".$j."','".$_SESSION['s_colorLinPar']."','".$_SESSION['s_colorLinImpar']."');\"  onMouseDown=\"marca('linhax".$j."','".$_SESSION['s_colorMarca']."'); redirect('".$_SERVER['PHP_SELF']."?action=popup&cod=".$row['scpt_id']."&cellStyle=true&bbutton=true')\">";			
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") )&& empty($_POST['submit'])) {
		...
		print "<TEXTAREA class='textarea' name='script' id='idScript'></textarea>";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<TEXTAREA class='textarea' name='script' id='idScript'>" . $row['scpt_script'] . "</textarea>";
			?>
			<script type="text/javascript">
