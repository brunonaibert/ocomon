...
print "<FORM name='form_usuarios' id='form_usuarios' method='POST' action='" . $_SERVER['PHP_SELF'] . "' onSubmit='return valida()'>";
...
print "<TABLE id='tb_main' border='0' " . $cellStyle . " align='center' width='100%' >";
...
//     if ($OPERADOR_AREA) {
//         $qryarea = "SELECT sis_id, sistema FROM sistemas WHERE sis_id = " . $_SESSION['s_area'] . " ";
//     }
// //OR sis_id = 2 -  2 - USUARIOS
...
//         $isSelecionado = "";
//         if ($rowArea['sis_id'] == (isset ($_SESSION['id_sistema_filtro']) ? $_SESSION['id_sistema_filtro'] : '')) {
...
//         "</td></tr>";
    ...
    if ($registros == 0) {
        ...
        // echo "<table id='tb_list' class='stripe hover order-column row-border' border='0' cellspacing='0' width='100%'>";
        ...
        // NOVA TABELA - TESTE
        print "<tr><td colspan='8' class='line'>";
        echo "<table id='table_users' class='stripe hover order-column row-border' border='0' cellspacing='0' width='100%'>";
...
if ((isset($_GET['action']) && ($_GET['action'] == "incluir")) && empty($_POST['submit'])) {
    ...
    print "<tr><td colspan='5' width='100%'>";
    ...
    print "<SELECT class='select' name='categoria' id='idCategoria' onChange=\"ajaxFunction('divAreas', 'areas_possiveis.php', 'idLoad', 'nivel=idCategoria');\">";
    ...
    if ($OPERADOR_AREA) {
        ...
        $query = "SELECT * from nivel WHERE nivel_cod <> 5 order by nivel_nome";
    }
    ...
    print "<SELECT class='select' name='area' size=1 id='idArea' disabled>";

    print "</SELECT>";
    print "<input type='checkbox' name='areaadmin' value=1 disabled>" . TRANS('COL_AREA_ADMIN') . "";
    ...
    // print "<a href='#' onClick=\"javascript:showElements('form_usuarios')\">DEBUG NO CONSOLE</a>";
    print "</TR>";
...
if ((isset($_GET['action']) && $_GET['action'] == "alter") && empty($_POST['submit'])) {
    ...
    print "<input type='hidden' name='login' id='idLogin' value='" . $row['login'] . "' >";

    print "<input type='hidden' name='user_id' id='user_id' value='" . $row['user_id'] . "'>";

    print "<TD width='20%' align='left' bgcolor='" . TD_COLOR . "'>" . TRANS('COL_LEVEL') . ":</TD>";

    if ($OPERADOR_AREA) //se for administrador da área poderá apenas alterar o nível para desabilitado
    {
        $qrynivel = "SELECT * from nivel WHERE nivel_cod IN (" . $_SESSION['s_nivel'] . ", 5) order by nivel_nome ";
    } else {
    ...
    print "<TD width='30%' align='left' bgcolor='" . BODY_COLOR . "'>";
    print "<SELECT class='select' name='categoria' id='idCategoria' onChange=\"ajaxFunction('divAreas', 'areas_possiveis.php', 'idLoad', 'nivel=idCategoria', 'usuario=user_id');\">";
    ...
    print "<TD width='20%' align='left' bgcolor='" . TD_COLOR . "'>" . TRANS('COL_PASS') . ":</TD>";
    print "<TD width='30%' align='left' bgcolor='" . BODY_COLOR . "'><INPUT type='password' class='text' name='password' id='idSenhaVazia' value=''></TD>";
    ...
    $sis_atende = (($row['nivel'] != 3 && $row['nivel'] != 5) ? 1 : 0);
    ...
    if ($OPERADOR_AREA) {
        ...
        $qryarea = "SELECT * from sistemas where sis_status not in (0) " . ($sis_atende == 0 ? ' AND sis_atende = 0 ' : ' AND sis_atende = 1') . " order by sistema";
    }
    ...
    print "<input type='checkbox' name='areaadmin' value=1 " . $check . " " . ($sis_atende == 0 ? 'disabled' : '') . " >" . TRANS('COL_AREA_ADMIN') . "";
    print "</TD>";
    print "</TR>";
    ...
    print "<TD colspan='2' align='center' width='50%' bgcolor='" . BODY_COLOR . "'><input type='submit' class='button' value='" . TRANS('BT_ALTER') . "' name='submit'>";
    print "</TD>";
    ...
    if ($sis_atende) {
        ...
        $qry_areas = "select * from usuarios_areas where uarea_uid=" . noHtml($_GET['login']) . "";
...
if ((isset($_GET['action']) && $_GET['action'] == "stat") && empty($_POST['submit'])) {
    ...
    print "<br/><br/><b><p align='center'>" . TRANS('TTL_USERS_BRIEF') . "</p></b><br/>";

    print "<table width='50%' cellspacing='0' align='center' style='border: 1px solid black; margin:auto'>";
    print "<tr class='header'><td class='line'>" . strtoupper(TRANS('COL_LEVEL')) . "</td><td class='line'>" . strtoupper(TRANS('COL_QTD')) . "</td></tr>";
    $TOTAL = 0;
    while ($rowStat = mysql_fetch_array($execStat)) {
        ...
        print "<tr><td class='line'>" . $rowStat['nivel'] . "</td><td class='line'>" . $rowStat['quantidade'] . "</td></tr>";
        $TOTAL += $rowStat['quantidade'];
    }
    print "<tr class='header'><td class='line' >" . TRANS('TOTAL') . "</td><td class='line'>" . $TOTAL . "</td></tr>";
    print "</table>";
    ...
    if (!$OPERADOR_AREA) {
        ...
        if ($registrosTmp > 0) {
            print "<br/><br/><b><p align='center'>" . TRANS('TTL_WAITING_CONFIRMATION') . "</p></b><br/>";
            print "<table width='50%' cellspacing='0' align='center' style='border: 1px solid black; margin:auto;'>";
            print "<tr class='header'><td class='line'>" . TRANS('COL_NAME') . "</td><td class='line'>" . TRANS('COL_LOGIN') . "</td><td class='line'>" . TRANS('COL_EMAIL') . "</td><td class='line'>" . TRANS('COL_CONFIRM') . "</td><td class='line'>" . TRANS('COL_DEL') . "</td></tr>";
            ...
            print "<tr class='header'><td class='line' colspan='2'><b>" . TRANS('TOTAL') . "</b></td><td class='line' colspan='3'><b>" . $registrosTmp . " " . TRANS('RECORDS') . "</b></td></tr>";
            print "</table>";

        } else {
            print "<p align='center'>" . TRANS('MSG_NO_PENDENCES') . "</p>";
        }
...
if ($_POST['submit'] == TRANS('BT_CAD')) {
    ...
    if (empty($_POST['login']) || empty($_POST['nome']) || empty($_POST['data_inc']) || empty($_POST['data_admis']) || empty($_POST['email']) || empty($_POST['telefone']) || empty($_POST['categoria']) || empty($_POST['area'])) {
        $erro = true;
...
/*            if ($_POST['areaadmin']){
        ...
        ",'" . $data_admis . "','" . $_POST['email'] . "','" . noHtml($_POST['telefone']) . "', " . $_POST['categoria'] . ", "
...
if ($_POST['submit'] == TRANS('BT_ALTER')) {
    ...
    if (!$erro) {
        $qrytesta = "SELECT * FROM sistemas where sis_id = " . noHtml($_POST['area']) . "";
        $execteste = mysql_query($qrytesta);
        $rowtesta = mysql_fetch_array($execteste);
    ...
    if (!$erro) {
        ...
        $newPass = (isset($_POST['password']) && !empty($_POST['password']) ? md5($_POST['password']) : 0);

        // $pass = md5($_POST['password']);

        if (!$newPass) {
            $query2 = "UPDATE usuarios SET nome='" . noHtml($_POST['nome']) . "', data_inc='" . $data_inc . "', " .
            "data_admis='" . $data_admis . "', email='" . $_POST['email'] . "', fone='" . noHtml($_POST['telefone']) . "'," .
                "nivel=" . $_POST['categoria'] . ", AREA=" . $_POST['area'] . ", user_admin='" . $areaadmin . "' " . "
						WHERE user_id=" . $_POST['login'] . "";
        } else {
            $query2 = "UPDATE usuarios SET nome='" . noHtml($_POST['nome']) . "', password='" . $newPass . "', " .
            "data_inc='" . $data_inc . "', data_admis='" . $data_admis . "', email='" . $_POST['email'] . "', " .
            "fone='" . noHtml($_POST['telefone']) . "', nivel=" . $_POST['categoria'] . ", AREA=" . $_POST['area'] . ", " .
...
$(document).ready( function () {
	...
	function valida(){
		...
		var ok = validaForm('idLogin','USUARIO','Login',1);
		if (ok) var ok = validaForm('idCategoria','COMBO','Nível',1);
		if (ok) var ok = validaForm('idNome','','Nome',1);

		if (ok) {
			if (senha != null) {
				var ok = validaForm('idSenha','ALFAFULL','Senha',1);
			}
		}
